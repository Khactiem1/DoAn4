    ...
    $scope.modal = function (state,id,name){
        ...
        switch (state){
            case 'add':
                ...
                $scope.frmTitle = "Thêm thương hiệu";
                break;
            case 'edit':
                document.getElementById('customFileExample5').innerText = 'Hình ảnh';
                $scope.frmTitle = "Sửa thương hiệu";
                $scope.id = id;
                $http.get(API + 'trademark/edit/' + id).then(function (res){
                    $scope.trademark = res.data;
                    document.getElementById("trademark-image").src = urlImage + res.data.image_path;
                });
                break;
            case 'delete':
                $scope.frmTitle = "Xoá thương hiệu";
                $scope.id = id;
                $scope.name = name;
                break;
            default:
        ...
        if(state == 'add'){
            if(isFormValid == true){
                if(document.getElementById('trademark-image').getAttribute("src") == null){
                    active__messageNotification("Hãy chọn hình ảnh cho thương hiệu",false);
                }
                else {
                    btnSubmit[0].classList.add('submit-loadding');
                    var url = API + 'trademark/add';
                    ...
                    $http({
                        ...
                        if(res.data.status == 'T'){
                            ...
                            active__messageNotification("Tên thương hiệu đã tồn tại",false);
                        }
                        else {
            ...
            if(isFormValid == true){
                ...
                $http({
                    ...
                    if(res.data.status == 'T'){
                        ...
                        active__messageNotification("Tên thương hiệu này đã tồn tại",false);
                    }
                    else {
                        active__messageNotification("Có lỗi xảy ra hãy thử lại sau",false);
                    }
            ...
            $http.get(API + 'trademark/delete/' + id).then(function (res) {
                ...
                if(res.data.status == 'T'){
                    ...
                    active__messageNotification("Thương hiệu này đang tồn tại trong danh sách sản phẩm không thể xoá",false);
                }
                else {
                    active__messageNotification("Có lỗi xảy ra hãy thử lại sau",false);
                }
